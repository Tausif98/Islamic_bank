...
class Settings(BaseSettings):
    """
    All config from .env file. Pydantic validates types at startup.
    If a required env var is missing → app crashes immediately (fail-fast).
    """
    ...
    mongodb_db_name: str = Field(default="islamic_bank", alias="MONGODB_DB_NAME")
    ...
    kafka_account_topic: str = Field(default="account-events", alias="KAFKA_ACCOUNT_TOPIC")
    kafka_transaction_topic: str = Field(default="transaction-events", alias="KAFKA_TRANSACTION_TOPIC")
    kafka_profit_topic: str = Field(default="profit-events", alias="KAFKA_PROFIT_TOPIC")
    kafka_fraud_topic: str = Field(default="fraud-alerts", alias="KAFKA_FRAUD_TOPIC")
    kafka_notification_topic: str = Field(default="notification-events", alias="KAFKA_NOTIFICATION_TOPIC")
